...
for (bam.file in bam.files){
  ...
  for (index.subset in names(index.subsets)){
    load(file.path("ProcessedData","UniqueTranscribedBasesCoverageRleTracks","bam.file",paste0("unique.transcribed.bases.coverage.track.list.",unlist(strsplit(index.subset,split = "_"))[2],".RData")))
...
save(human.refseq.major.isoform.exon.coverage.antisense.corrected,file="ProcessedData/human.refseq.major.isoform.exon.coverage.antisense.corrected.RData")
...
for (bam.file in bam.files){
  ...
  for (index.subset in names(index.subsets)){
    load(load(file.path("ProcessedData","UniqueTranscribedBasesCoverageRleTracks","bam.file",paste0("unique.transcribed.bases.coverage.track.list.",unlist(strsplit(index.subset,split = "_"))[2],".RData")))
